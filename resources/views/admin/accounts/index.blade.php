...
@section('content')
        ...
        <div class="card-body">
          <div class="row no-gutters align-items-center">
            <div class="col mr-2">
              ...
              <div class="h5 mb-0 font-weight-bold text-gray-800" id="total-accounts-count">{{ $totalAccountsCount ?? 0 }}
              </div>
            </div>
            <div class="col-auto">
        ...
        <div class="card-body">
          <div class="row no-gutters align-items-center">
            <div class="col mr-2">
              ...
              <div class="h5 mb-0 font-weight-bold text-gray-800" id="locked-accounts-count">
        ...
        <div class="card-body">
          <div class="row no-gutters align-items-center">
            <div class="col mr-2">
              ...
              <div class="h5 mb-0 font-weight-bold text-gray-800" id="unverified-accounts-count">
    ...
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <button type="button" class="btn btn-info btn-sm me-2" data-bs-toggle="modal"
          data-bs-target="#manageAccountTypesModal">
        ...
        <div class="card card-body mb-3">
          <div class="row g-3">
            <div class="col-md-3">
              ...
              <input type="text" class="form-control form-control-sm column-filter" placeholder="Tìm kiếm..."
                data-column="1" id="filter-name">
            </div>
            <div class="col-md-3">
              <label class="form-label fw-bold small">Email</label>
              <input type="text" class="form-control form-control-sm column-filter" placeholder="Tìm kiếm..."
                data-column="2" id="filter-email">
            </div>
            <div class="col-md-2">
          ...
          <tbody>
            @foreach($accounts as $account)
              <tr>
                ...
                <td>
                  <div class="user-info">
                    <img src="{{ $account->avatar_url ?: asset('images/man.jpg') }}" alt="Avatar" class="user-avatar"
                      onerror="this.src='{{ asset('images/man.jpg') }}'" loading="lazy">
                ...
                <td>
                  <div class="btn-group" role="group">
                    <button class="btn btn-primary btn-sm edit-account-btn" data-account-id="{{ $account->account_id }}"
                      data-fullname="{{ $account->profile->fullname ?? '' }}" data-email="{{ $account->email }}"
                      data-account-type-id="{{ $account->account_type_id }}" title="Chỉnh sửa">
                      <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-danger btn-sm delete-account-btn" data-account-id="{{ $account->account_id }}"
                      title="Xóa">
      ...
      <div class="modal-content">
        ...
        <form id="editAccountForm" method="POST">
          ...
          <div class="modal-body">
            <div class="mb-3">
              <label class="form-label">Họ và tên</label>
              <input type="text" class="form-control" id="edit-fullname" name="fullname" readonly
  ...
  <div class="modal fade" id="manageAccountTypesModal" tabindex="-1" aria-labelledby="manageAccountTypesModalLabel"
    aria-hidden="true">
              ...
              <form id="addAccountTypeForm">
                <div class="row">
                  <div class="col-md-12">
                    ...
                    <input type="text" class="form-control" id="newAccountTypeName" name="name" required
                ...
                <div class="row mt-3">
                  <div class="col-12">
                    ...
                    <textarea class="form-control" id="newAccountTypeDescription" name="description" rows="2"
                      placeholder="Mô tả về loại tài khoản này..."></textarea>
                  </div>
                </div>
                <div class="mt-3">
                  <button type="submit" class="btn btn-primary">
                      ...
                      <tr data-type-id="{{ $type->account_type_id }}">
                        ...
                        <td>
                          <button class="btn btn-sm btn-outline-primary edit-type-btn"
                            data-type-id="{{ $type->account_type_id }}" data-type-name="{{ $type->name }}"
                            data-type-description="{{ $type->description ?? '' }}"
                            data-type-withdraw="{{ $type->withdraw_monthly_limit_cents }}">
                            <i class="fas fa-edit"></i>
                          </button>
  ...
  <div class="modal fade" id="editAccountTypeModal" tabindex="-1" aria-labelledby="editAccountTypeModalLabel"
    aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <form id="editAccountTypeForm">
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">Hạn mức rút tiền hàng tháng (VNĐ)</label>
              <input type="number" class="form-control" id="edit-withdraw-monthly" name="withdraw_monthly_limit_cents"
                min="0" step="1000" placeholder="Nhập hạn mức, ví dụ 5000000">
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.edit-type-btn', function () {
        const btn = this; // DOM element
        const typeId = btn.dataset.typeId;
        const typeName = btn.dataset.typeName;
        const typeDescription = btn.dataset.typeDescription || '';
        const withdrawLimit = btn.dataset.typeWithdraw || 0;

        $('#edit-type-id').val(typeId);
        $('#edit-type-name').val(typeName);
        $('#edit-type-description').val(typeDescription);
        $('#edit-withdraw-monthly').val(withdrawLimit);

        // Nếu dùng Bootstrap 5:
        const el = document.getElementById('editAccountTypeModal');
      ...
      $('#editAccountTypeForm').on('submit', function (e) {
        ...
        $.ajax({
          ...
          data: {
            ...
            withdraw_monthly_limit_cents: $('#edit-withdraw-monthly').val()
          },
          headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          },
          success: function (response) {
            ...
            if (response.success) {
              ...
              const newLimit = response.accountType.withdraw_monthly_limit_cents;
              ...
              editBtn.attr('data-type-withdraw', newLimit);
