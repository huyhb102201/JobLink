          ...
          <tbody>
            @forelse($rows as $r)
              <tr>
                ...
                <td class="text-center">
                  ...
                  @elseif($st === 2)
                    <span class="badge text-bg-secondary">
                      <i class="fas fa-lock me-1"></i>Đã xử lý
                    </span>
                  @elseif($st === 0)
                ...
                <td class="text-center">
                  <button class="btn btn-sm btn-primary btn-view" data-job-id="{{ $r->job_id }}"
                    data-job-title="{{ $r->job_title ?? '' }}" data-owner-name="{{ $r->owner_name }}"
                    data-owner-email="{{ $r->owner_email }}" data-owner-id="{{ $r->owner_id ?? '' }}"
            ...
            @empty
              <tr>
                <td colspan="7" class="text-center text-muted py-4">Không có job nào bị báo cáo.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header text-white"
          style="background:linear-gradient(135deg,var(--jl-primary),var(--jl-primary-2));">
        ...
        <div class="modal-body">
          ...
          <div class="d-flex flex-wrap gap-2 mb-3">
            ...
            <button id="btnRejectReports" class="btn btn-outline-secondary">
              <i class="fa-solid fa-ban me-1"></i> Từ chối báo cáo
            </button>
...
@push('scripts')
    ...
    $(function () {
      ...
      function cardReporter(r) {
        ...
        const items = (r.reports || []).map(rep => {
          const imgs = (rep.images || []).map(u => `
    <span class="img-wrap m-1">
      <img src="${u}" data-src="${u}" alt="Ảnh báo cáo" width="110" height="110"
           class="report-img"
           style="object-fit:cover;border-radius:8px;border:1px solid #e5e7eb"
           loading="lazy" onerror="this.style.opacity=0.3">
      <button type="button" class="btn btn-light btn-sm btn-zoom rounded-circle shadow-sm"
              title="Phóng to">
  ...
  `).join('');
      ...
      $('#btnDeleteJob').on('click', async function () {
        ...
        if (Number(currentJobStatus) !== 1) {             // ✅ guard
          return Swal.fire({ icon: 'info', title: 'Không thể thao tác', text: 'Chỉ thao tác khi trạng thái là "Chờ xử lý".' });
        }
      ...
      $('#btnRejectReports').on('click', async function () {
        if (!currentJobId) {
          return Swal.fire({ icon: 'info', title: 'Thiếu thông tin', text: 'Không tìm thấy ID job.' });
        }
        if (Number(currentJobStatus) !== 1) {             // ✅ guard
          return Swal.fire({ icon: 'info', title: 'Không thể thao tác', text: 'Chỉ thao tác khi trạng thái là "Chờ xử lý".' });
        }
        ...
        const ok = await Swal.fire({
          icon: 'question',
          title: 'Từ chối các báo cáo về job này?',
          html: `Job ID: <b>#${currentJobId}</b><br><small>Hành động này sẽ đánh dấu các báo cáo là <b>đã xử lý (từ chối)</b>.</small>`,
        ...
        toggleActionHint(true);
        try {
          const url = `/admin/reported-jobs/${currentJobId}/reject`;
          ...
          await Swal.fire({ icon: 'success', title: 'Đã từ chối báo cáo!', timer: 1400, showConfirmButton: false });

          // Cập nhật trạng thái ngay trên bảng
          const $row = $('#reportedJobsTable button[data-job-id="' + currentJobId + '"]').closest('tr');
          // Cột trạng thái là cột thứ 6 (index 5)
          $row.find('td').eq(5).html('<span class="badge text-bg-danger"><i class="fas fa-ban me-1"></i>Đã từ chối</span>');
          $row.find('.btn-view').data('status', 0);
          currentJobStatus = 0;
          updateActionButtons();
          // Nếu muốn ẩn modal sau khi thao tác:
          bootstrap.Modal.getInstance(document.getElementById('reportersModal')).hide();
        } catch (e) {
          Swal.fire({ icon: 'error', title: 'Lỗi', text: e.message || 'Không thể từ chối báo cáo.' });
        } finally {
          toggleActionHint(false);
        }
      });
    ...
    let currentJobId = null;
    let currentOwnerId = null;
    let currentJobStatus = null;   // ✅ thêm

    function updateActionButtons() {
      const isPending = Number(currentJobStatus) === 1;
      $('#btnDeleteJob, #btnLockAccount, #btnRejectReports')
        .prop('disabled', !isPending)
        .toggleClass('disabled', !isPending)
        .attr('title', isPending ? '' : 'Chỉ thao tác khi trạng thái là "Chờ xử lý"');
    }
    // Phóng to ảnh: click ảnh hoặc nút kính lúp
    $('#md_list').on('click', '.report-img, .btn-zoom', function (e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      Swal.fire({
        imageUrl: src,
        imageAlt: 'Ảnh báo cáo',
        showConfirmButton: false,
        showCloseButton: true,
        background: '#000',
        width: 'auto',
        padding: 0,
        customClass: { popup: 'sw-image-popup' }
      });
    });
