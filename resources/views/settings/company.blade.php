    ...
    <div class="col-12">
      ...
      @if(!$isBusiness)
        <div class="surface p-4 rounded-4">
          ...
          <div class="text-secondary">Nâng cấp lên gói Business để tạo doanh nghiệp, mời thành viên và phân quyền.</div>
        </div>
      ...
      @elseif(!$org)
        ...
        <div class="empty-light rounded-4 p-5 text-center">
          ...
          <button class="btn btn-primary btn-lg rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#createOrgModal">
              ...
              <div class="list-group-item px-4 py-3">
                <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
                  ...
                  <div class="d-flex align-items-center gap-3">
                    ...
                    @endif
                    <span class="badge rounded-pill role-badge {{ $roleClass }}">{{ ucfirst(strtolower($role)) }}</span>
                    <span class="text-secondary small">
                      @if($state === 'ACTIVE')
                        Tham gia {{ \Carbon\Carbon::parse($m->joined_at)->format('d/m/Y') }}
                      @else
                        Đã mời {{ \Carbon\Carbon::parse($m->invited_at ?? $m->joined_at)->format('d/m/Y H:i') }}
                      @endif
                    </span>
        ...
        <div class="modal-body pt-3">
          ...
          <div class="row g-3">
            ...
            <div class="col-12 col-md-6">
              ...
              <div class="d-flex align-items-center gap-2">
                <input type="number" name="seats_limit" class="form-control w-auto" style="max-width:120px" min="1" max="5" required value="{{ old('seats_limit', 5) }}">
            ...
            <div class="col-12">
              ...
              <textarea name="description" class="form-control" rows="3" placeholder="Mô tả ngắn về doanh nghiệp">{{ old('description') }}</textarea>
        ...
        <form method="POST" action="{{ route('company.members.invite') }}">
          ...
          <div class="modal-body">
            ...
            <div class="input-group">
              ...
              <input type="text" name="username" class="form-control" required placeholder="vd: nguyenvana" value="{{ old('username') }}">
            </div>
            <div class="form-text">Người được mời cần xác nhận qua email. Quyền mặc định: <strong>Member</strong>.</div>
...
@push('styles')
<style>
  .surface{background:#fff;border:1px solid #edf0f4;box-shadow:0 2px 12px rgba(30,41,59,.04)}
  .empty-light{background:linear-gradient(180deg,#f9fbff 0%,#fff 100%);border:1px dashed #d8e0ef;}
  .empty-icon{width:72px;height:72px;border-radius:20px;background:#eff4ff;color:#4f46e5;display:grid;place-items:center;font-size:30px;}
  .overview-card{background:#fff;border:1px solid #edf0f4;box-shadow:0 6px 20px rgba(30,41,59,.06);}
  .chip-light{display:inline-block;padding:.35rem .75rem;border-radius:999px;background:#f3f6ff;color:#334155;font-size:.875rem;border:1px solid #e5eaff;}
  .shadow-xs{box-shadow:0 4px 16px rgba(30,41,59,.05);}
  .avatar{width:40px;height:40px;border-radius:50%;background:#e6ecff;color:#233876;display:grid;place-items:center;font-weight:700;}
  .role-badge{padding:.4rem .75rem;font-weight:600;}
  ...
  .badge-member{background:#f1f5f9;color:#0f172a;}
  .badge-billing{background:#ffe4e6;color:#9f1239;}
  .btn-icon{width:34px;height:34px;padding:0;display:inline-grid;place-items:center;border-radius:999px;line-height:1;}
  .btn-soft-danger{background:#fff5f5;border:1px solid #ffe0e0;color:#c1121f;}
  .btn-soft-danger:hover{background:#ffe5e5;border-color:#ffc9c9;color:#a50e19;box-shadow:0 0 0 .2rem rgba(220,53,69,.12);}
  .btn-soft-danger:active{transform:translateY(1px);}
  .verify-badge{border-radius:999px;font-weight:600;padding:.25rem .6rem;display:inline-flex;align-items:center;gap:.35rem;font-size:.8rem;border:1px solid transparent;}
  ...
  .verify-thumb{width:48px;height:36px;object-fit:cover;border-radius:8px;border:1px solid #e5e7eb;box-shadow:0 1px 2px rgba(0,0,0,.05);display:block;}
  .verify-thumb:hover{transform:scale(1.03);box-shadow:0 4px 10px rgba(0,0,0,.08);}
</style>
@endpush
