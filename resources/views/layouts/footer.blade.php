<footer id="footer" class="footer dark-background">

  <div class="footer-top">
    <div class="container">
      <div class="row gy-4">

        <div class="col-lg-4 col-md-6 footer-about">
          <a href="index.html" class="logo d-flex align-items-center">
            <span class="sitename">JobLink</span>
          </a>
          <div class="footer-contact pt-3">
            <p>Địa chỉ: Số 73, Nguyễn Huệ,</p>
            <p>Phường Long Châu, Tp. Vĩnh Long</p>
            <p class="mt-3"><strong>Điện thoại:</strong> <span>+84 963 887 651</span></p>
            <p><strong>Email:</strong> <span>22004046@st.vlute.edu.vn</span></p>
          </div>
        </div>

        <div class="col-lg-2 col-md-3 footer-links">
          <h4>Liên kết hữu ích</h4>
          <ul>
            <li><a href="{{ route('home') }}">Trang chủ</a></li>
            <li><a href="#">Về chúng tôi</a></li>
            <li><a href="#">Dịch vụ</a></li>
            <li><a href="#">Điều khoản</a></li>
            <li><a href="#">Chính sách bảo mật</a></li>
          </ul>
        </div>

        <div class="col-lg-2 col-md-3 footer-links">
          <h4>Dịch vụ của chúng tôi</h4>
          <ul>
            <li><a href="#">Thiết kế web</a></li>
            <li><a href="#">Phát triển web</a></li>
            <li><a href="#">Quản lý sản phẩm</a></li>
            <li><a href="#">Marketing</a></li>
            <li><a href="#">Thiết kế đồ họa</a></li>
          </ul>
        </div>

        <div class="col-lg-2 col-md-3 footer-links">
          <h4>Thông tin khác</h4>
          <ul>
            <li><a href="#">Chính sách</a></li>
            <li><a href="#">Hướng dẫn</a></li>
            <li><a href="#">FAQ</a></li>
            <li><a href="#">Blog</a></li>
            <li><a href="#">Liên hệ</a></li>
          </ul>
        </div>

        <div class="col-lg-2 col-md-3 footer-links">
          <h4>Tài nguyên</h4>
          <ul>
            <li><a href="#">Tài liệu hướng dẫn</a></li>
            <li><a href="#">Báo cáo & Nghiên cứu</a></li>
            <li><a href="#">Video hướng dẫn</a></li>
            <li><a href="#">Webinar & Sự kiện</a></li>
            <li><a href="#">Công cụ miễn phí</a></li>
          </ul>
        </div>
      </div>
    </div>
  </div>

  <div class="copyright text-center">
    <div
      class="container d-flex flex-column flex-lg-row justify-content-center justify-content-lg-between align-items-center">

      <div class="d-flex flex-column align-items-center align-items-lg-start">
        <div>
          © Bản quyền <strong><span>MyWebsite</span></strong>. Đã đăng ký bản quyền
        </div>
        <div class="credits">
          Thiết kế bởi <a href="https://huyhogia.id.vn/">GiaHuyTV</a> - Phân phối bởi <a
            href="https://daotao.vlute.edu.vn/">VLUTE</a>
        </div>
      </div>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">

      <div class="social-links order-first order-lg-last mb-3 mb-lg-0">
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=22004027@st.vlute.edu.vn" target="_blank">
          <i class="fa-brands fa-google"></i>
        </a>
        <a href="https://www.facebook.com/nguyen.thanh.hoang.john.nguyen" target="_blank">
          <i class="bi bi-facebook"></i>
        </a>
        <a href="https://github.com/huyhb102201" target="_blank">
          <i class="fa-brands fa-github"></i>
        </a>
      </div>


    </div>
  </div>

</footer>