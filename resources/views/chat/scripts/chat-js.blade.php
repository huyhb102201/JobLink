{{-- resources/views/chat/scripts/chat-js.blade.php --}}
<script>
let currentChat = '';
    ...
    if (!isMe) {
        avatarHTML = `
            <div class="avatar-container">
                <img src="${msg.sender.avatar_url ?? window.chatConfig.defaultAvatar}" 
                     alt="Sender Avatar">
            </div>
        `;
    } else {
    ...
    if (msg.img) {
        contentHTML += `
            <img src="${msg.img}" class="message-img" alt="Message Image"
                 onerror="this.src='${window.chatConfig.defaultAvatar}'; this.onerror=null; console.error('Image load failed:', '${msg.img}');">
        `;
    }
    ...
    if (!isMe && (currentPartnerId === incomingMsg.sender_id || currentJobId === incomingMsg.job_id || currentOrgId === incomingMsg.org_id)) {
        ...
        if (msgDiv) {
            ...
            window.showChatToast(
                ...
                incomingMsg.sender.avatar_url ?? window.chatConfig.defaultAvatar
            );

            window.sendBrowserNotification(
                "Tin nhắn mới từ " + incomingMsg.sender.name,
                incomingMsg.content || '[Hình ảnh]',
                incomingMsg.sender.avatar_url ?? window.chatConfig.defaultAvatar,
                `chat-${incomingMsg.id}`
            );
...
function openBoxChat(name, element, partnerId, boxId, jobId = null, orgId = null) {
    ...
    if (partnerId) {
        ...
        const avatarUrl = avatarImg ? avatarImg.src : window.chatConfig.defaultAvatar;
...
function sendMessage() {
    ...
    const tempMsg = {
        ...
        sender: { id: window.authId, name: 'Bạn', avatar_url: '{{ auth()->user()->avatar_url ?? window.chatConfig.defaultAvatar }}' },
